...
void run_test(const std::string& StdArg, const std::string& ErrArg, const k3d::system::spawn_type SpawnType)
{
	...
	if(!k3d::system::spawn(boost::filesystem::path("./spawned-test-command"), args, SpawnType, std_out, std_err, boost::filesystem::path("../../bin")))
	{
...
void run_test_env(const std::string& EnvValue)
{
	...
	if(!k3d::system::spawn(boost::filesystem::path("./spawned-test-command"), args, k3d::system::SPAWN_SYNCHRONOUS, std_out, std_err, boost::filesystem::path("../../bin"), env))
	{
		throw std::runtime_error("error spawning command");
	}

...
public:
		...
		m_node_selection(init_owner(*this) + init_name("node_selection") + init_label(_("Node Selection")) + init_description(_("Node storing the currently selected nodes")) + init_value(static_cast<k3d::inode_selection*>(0)))
	{
		...
		m_point_size.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_background_color.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_fog.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_fog_near.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_fog_far.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_headlight.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_show_lights.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_draw_two_sided.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_draw_safe_zone.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_draw_aimpoint.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_draw_crop_window.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_draw_frustum.changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		m_node_selection.changed_signal().connect(boost::bind(&render_engine::on_node_selection_changed, this, _1));
	}
	...
	{
		m_redraw_request_signal(k3d::gl::irender_viewport::ASYNCHRONOUS);
	}
	...
	void on_node_selection_changed(k3d::iunknown*)
	{
		...
		if(node_selection)
		{
			m_selection_changed_connection = node_selection->selection_changed_signal().connect(boost::bind(&render_engine::on_redraw, this, _1));
		}
	}
...
private:
	...
	boost::signals2::connection m_selection_changed_connection;
};

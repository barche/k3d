...
class ishader_property_factory : public k3d::property::iproperty_factory
{
public:
	iproperty* create(inode &Owner, const string_t &Name, const string_t &Label, const string_t &Description, const boost::any &Value)
	{
		...
		ishader* initial_value = boost::any_cast<ishader*>(Value);

		typedef k3d::property::user_property<k3d_data(ishader*, immutable_name, change_signal, with_undo, node_storage, no_constraint, node_property, property::user_node_serialization)> property_t;
...
struct ishader_registrator
{
	ishader_registrator()
	{
		const string_t name = "k3d::gl::ishader";
		k3d::register_type(typeid(k3d::gl::ishader), name);
		k3d::register_type(typeid(k3d::gl::ishader*), name + "*");
		k3d::property::register_property_factory(name + "*", std::shared_ptr<k3d::property::iproperty_factory>(new ishader_property_factory()));
	}
};

ishader_registrator register_ishader;
